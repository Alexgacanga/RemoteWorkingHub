        ...
        <div class="w-full max-w-lg bg-[#FFFFFF] rounded-2xl shadow-lg p-6 sm:p-8 relative">
            ...
            <div class="mb-8">
                <h2 class="text-2xl sm:text-3xl font-bold text-[#000000] mb-2 tracking-tight leading-tight">
                    Create day-pass for <span class="text-[#FF6245]">{{ $customer->fname }}</span>
                </h2>
                <p class="text-sm text-gray-500">Configure the day-pass duration and package details.</p>
            </div>
            @if ($errors->any())
            ...
            @endif
            <!-- Form -->
            <form action="{{ route('day-pass-subscriptions.store', $customer->id) }}" method="POST">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="number" id="duration_days" name="no_of_days" min="1" value="1" required
                            class="w-full px-4 py-3 bg-[#FFFFFF] border border-gray-300 rounded-lg text-[#000000] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#4FC1FF] focus:border-transparent transition-all duration-300 shadow-sm">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select id="package_id" name="package_id" required
                                ...
                                <option value="{{ $package->id }}">{{ $package->name }} - Ksh. {{ number_format($package->price, 2) }}/day</option>
                                @endforeach
                            </select>
