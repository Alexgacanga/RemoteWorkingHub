        ...
        <div class="w-full max-w-lg bg-[#FFFFFF] rounded-2xl shadow-lg p-6 sm:p-8 relative">
            ...
            <div class="mb-8">
                <h2 class="text-2xl sm:text-3xl font-bold text-[#000000] mb-2 tracking-tight leading-tight">
                    Edit day-pass for <span class="text-[#FF6245]">{{ $subscription->customer->fname }}</span>
                </h2>
                <p class="text-sm text-gray-500">Edit the day-pass duration and package details.</p>
            </div>
            @if ($errors->any())
            ...
            @endif
            <!-- Form -->
            <form action="{{ route('day-pass-subscriptions.store', $subscription->id) }}" method="PATCH">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="date" id="start_date" name="start_date" required
                            class="w-full px-4 py-3 bg-[#FFFFFF] border border-gray-300 rounded-lg text-[#000000] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#4FC1FF] focus:border-transparent transition-all duration-300 shadow-sm" value="{{ old('start_date', $subscription->start_date) }}">
                    </div>

                    <!-- Number of Days Input -->
                    <div>
                        <label for="duration_days" class="block text-sm font-semibold text-[#000000] mb-2">
                            Number of Days
                        </label>
                        <input type="number" id="duration_days" name="no_of_days" min="1" value="{{ old('no_of_days', $subscription->no_of_days) }}" required
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select id="package_id" name="package_id" required
                                ...
                                <option value="{{ $package->id }}" {{ old('package_id', $subscription->package_id) == $package->id ? 'selected' : '' }}>{{ $package->name }} - Ksh. {{ number_format($package->price, 2) }}/day</option>
                                @endforeach
                            </select>
