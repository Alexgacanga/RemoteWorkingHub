        ...
        <div class="w-full max-w-lg bg-[#FFFFFF] rounded-2xl shadow-lg p-6 sm:p-8 relative">
            ...
            <div class="mb-8">
                <h2 class="text-2xl sm:text-3xl font-bold text-[#000000] mb-2 tracking-tight leading-tight">
                    Edit weekly subscription for <span class="text-[#FF6245]">{{ $subscription->customer->fname }}</span>
                </h2>
                <p class="text-sm text-gray-500">Edit the package details.</p>
            </div>
            @if ($errors->any())
            ...
            @endif
            <!-- Form -->
            <form action="{{ route('weekly-subscriptions.store', $subscription->id) }}" method="PATCH">
                @csrf
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select id="package_id" name="package_id" required
                                ...
                                <option value="{{ $package->id }}" {{ old('package_id', $subscription->package_id) == $package->id ? 'selected' : '' }}>{{ $package->name }} - Ksh. {{ number_format($package->price, 2) }}/week</option>
                                @endforeach
                            </select>
